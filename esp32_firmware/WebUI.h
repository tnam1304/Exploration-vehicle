...
<body>
  ...
  <script>
    ...
    // ==========================================
    /* Đổi mã trạng thái STM32 thành nhãn ngắn trên web. */
    function safetyStateText(data) {
      const ra = Number(data.ra);
      const rear = Number(data.rear);

      /* Mã enum RA_State_t của STM32. */
      if (Number(data.warn) === 2) return "FIRE ALARM";
      if (ra === 9) return "SONAR ERR";
      if (ra === 10) return "ENC ERR";
      if (ra === 11) return "CONFIG ERR";
      if (ra === 4) return "AEB STOP";
      if (ra === 5) return "AEB HOLD";
      if (ra === 3) return "PRE BRAKE";
      if (ra === 2) return "FCW WARN";
      if (ra === 8) return "REV DANGER";
      if (ra === 7) return "REV WARN";

      /* Mã RC_State_t: 4 WARNING, 5 DANGER. */
      if (rear === 5) return "REAR DANGER";
      if (rear === 4) return "REAR WARN";
      return "SAFE";
    }

    function safetyStateColor(label) {
      if (label === "SAFE") return "#1b5e20";
      if (label === "SONAR ERR" || label === "ENC ERR" || label === "CONFIG ERR") return "#6a1b9a";
      if (label === "FCW WARN" || label === "REV WARN" || label === "REAR WARN") return "#e65100";
      return "#b71c1c";
    }
      ...
      fetch('/data', { cache: "no-store" })
        ...
        .then(data => {
          ...
          if(!voiceTimerActive) {
            ...
            const label = safetyStateText(data);
            sBar.innerText = label;
            sBar.style.backgroundColor = safetyStateColor(label);
          }
