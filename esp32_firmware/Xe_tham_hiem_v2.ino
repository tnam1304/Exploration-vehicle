...
volatile int   ra_state_val = 0;
volatile int   rear_state_val = 0;
...
static esp_err_t data_handler(httpd_req_t *req) {
  ...
  snprintf(json, sizeof(json),
           "{\"temp\":%d, \"dist\":%d, \"warn\":%d, \"enc\":%ld, \"spd\":%.1f, \"trav\":%.1f, \"ra\":%d, \"rear\":%d}",
           temp_val, dist_val, warn_val, enc_val, speed_val, travel_dist,
           ra_state_val, rear_state_val);
  ...
  if (httpd_req_get_url_query_str(req, buf, sizeof(buf)) == ESP_OK) {
    ...
    if (httpd_query_key_value(buf, "val", val, sizeof(val)) == ESP_OK) {
      ...
      if (current_cmd != new_cmd || new_cmd == 'S') {
        current_cmd = new_cmd;
        /* Parser STM32 nhận trực tiếp một ký tự điều hướng. */
        Serial.write((uint8_t)current_cmd);
      }
    }
  }
  ...
  while (Serial.available() > 0) {
    ...
    if (c == '\n') {
      ...
      if (rx_idx > 0) {
        ...
        int parsed = sscanf(rx_buf, "T%d;D%d;W%d;E%ld;S%d;M%d;A%d;R%d",
...
void loop() {
  ...
  if (current_cmd != 'S' && (millis() - last_heartbeat_time > 1200)) {
    current_cmd = 'S'; 
    /* Gửi lệnh dừng trực tiếp cho parser STM32. */
    Serial.write((uint8_t)current_cmd);
  }
  ...
  if (millis() - last_cmd_time > 50) {
    /* Heartbeat chỉ là ký tự lệnh, không thêm frame C:. */
    Serial.write((uint8_t)current_cmd);
