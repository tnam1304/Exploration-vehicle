...
void UART_Send_Telemetry(int16_t raw_t, uint32_t dist_cm, int warn,
    ...
    /* A là state chính, R là state va chạm phía sau. */
    int len = sprintf(tx_buffer, "T%d;D%lu;W%d;E%ld;S%d;M%d;A%d;R%d\n",
                      temp_c, (unsigned long)d_send, warn, (long)enc, spd_int, trav_int,
                      ra_state, rear_state);
